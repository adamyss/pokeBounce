...
# fish broken rn ballies.append(fibonaciBall.fish())
one = None
two = None
def starterUi(screen,over,first,indexOne,indexTwo,wins,losses):
    ...
    for ballLooper in ballies:
        ...
        xPos = 300 + (175 * xcounter) + ballLooper.spriteOffset[0]
        yPos = height/2 + ballLooper.spriteOffset[1] -275+(175 * valy)
        screen.blit(imgy,[xPos,yPos])
        winCount = font.render(str(wins[num]),True,(50,255,50))
        lossCount = font.render(str(losses[num]),True,(255,50,50))
        screen.blit(winCount,[xPos - 10,yPos +ballLooper.size * 1.5])
        screen.blit(lossCount,[xPos + ballLooper.size * 2 -10,yPos +ballLooper.size * 1.5])
...
def main():
    ...
    with open('skibidi.txt','r') as skib:
        with open('temp.txt','w') as tempy:
            counter = 1
            for line in skib: 
                tempy.write(str(line))
            ...
            if counter/2 < len(ballies):
                tempy.write("\n")
                for i in range(int(len(ballies) - counter / 2) + 1):        
                    tempy.write("wins:0\n")
                    tempy.write("losses:0\n")
                    print("adding")
            counter += 1
    shutil.copyfile('temp.txt','skibidi.txt')
    while starty == True:
        screen.blit(bgImg,[0,0])
        selectedIndexOne,selectedIndexTwo,starty,forceQuit,valy,valyTwo = starterUi(screen,valy,valyTwo,selectedIndexOne,selectedIndexTwo,wins,lossesy)
        pygame.display.flip()
    ...
    while running and forceQuit == False:
        ...
        if healthOne <= 0:
            running = False
            with open('skibidi.txt','r') as u:
                linseys = u.readlines()
                lineyy = linseys[selectedIndexTwo * 2 + 1]
                numb = int(lineyy.strip("wins:"))
                noNumb = lineyy.strip(str(numb))
                noNumb = noNumb.strip("\n")
                noNumb = noNumb.strip(str(numb))
                valy= noNumb+str(numb) + "\n"
                numb = int(numb+ 1)
                valy= noNumb+str(numb) + "\n"
                print(f'noNumb {noNumb} numby { numb}')
                print(f"VALY{valy}")
                linseys[selectedIndexTwo * 2 + 1] = valy
                lineyy = linseys[selectedIndexOne * 2]
                numb = int(lineyy.strip("losses:"))
                print(f'num {numb}')
                noNumb = lineyy.strip(str(numb))
                noNumb = noNumb.strip("\n")
                noNumb = noNumb.strip(str(numb))
                ...
                valy= noNumb+str(numb) + "\n"
                print(f"VALY{valy}")
                linseys[selectedIndexOne * 2] = valy
            with open('skibidi.txt','w') as new:
                new.writelines(linseys)
        elif healthTwo <= 0:
            running = False
            playerOneWins = True
            with open('skibidi.txt','r') as u:
                linseys = u.readlines()
                lineyy = linseys[selectedIndexOne * 2 + 1]
                numb = int(lineyy.strip("wins:"))
                noNumb = lineyy.strip(str(numb))
                noNumb = noNumb.strip("\n")
                noNumb = noNumb.strip(str(numb))
                ...
                noNumb = noNumb.strip("\n")
                noNumb = noNumb.strip(str(numb))
                linseys[selectedIndexOne * 2 +1] = valy
                lineyy = linseys[selectedIndexTwo * 2]
                numb = int(lineyy.strip("losses:"))
                noNumb = lineyy.strip(str(numb))
                noNumb = noNumb.strip("\n")
                noNumb = noNumb.strip(str(numb))
                numb = int(numb+ 1)
                valy= noNumb+str(numb) + "\n"
                linseys[selectedIndexTwo * 2] = valy
            with open('skibidi.txt','w') as new:
                new.writelines(linseys)
...
# this is due to Anirvinya carrying cause hes awesomeU and it stops the main stuff from getting called on imports
